...
if dfcL ~= 0 || dfcR ~= 0
    if ~strcmp(expType,'STB') && ~strcmp(expType,'SBF')
        error(['ExpLMStrackingExpOnly: WARNING! dfcL=[' num2str(dfcL) '] and dfcR=[' num2str(dfcR) '] are INVALID inputs for expType=' expType]);
    end
end
...
if ~isempty(find(vdfL ~= 0, 1)) || ~isempty(find(vdfR ~= 0, 1))
    if ~strcmp(expType,'VDF') && ~strcmp(expType,'SBF')
        error(['ExpLMStrackingExpOnly: WARNING! vdfL=[' num2str(vdfL) '] and vdfR=[' num2str(vdfR) '] are INVALID inputs for expType=' expType]);
    end
end
% VALID NDF INPUTS? (IF ANY ARE NON-ZERO...) ONE PAIR PER RUN ONLY
if ndfL ~= 0 || ndfR ~= 0
    if ~strcmp(expType,'LVF') && ~strcmp(expType,'LSB')
        error(['ExpLMStrackingExpOnly: WARNING! ndfL=[' num2str(ndfL) '] and ndfR=[' num2str(ndfR) '] are INVALID inputs for expType=' expType]);
    end
end
...
%     error(['ExpLMStrackingExpOnly: WARNING! mchL=[' num2str(mchL) '] and mchR[=' num2str(mchR) '] are INVALID inputs for expType=' expType]);
%     end
% end
% CHECK MATLEAP SIGNED COORDINATE AXES
if numel(axSignMATLEAP) ~= 3 || (~isequal(axSignMATLEAP,[1 1 -1]) & ~isequal(axSignMATLEAP,[-1 -1 -1]))
    error(['ExpLMStrackingExpOnly: WARNING! axSignMATLEAP must equal [1 1 -1] for rightside up) OR [-1 -1 -1] for upside down. axSignMATLEAP=[' num2str(axSignMATLEAP) ']']);
else
...
if ~isequal(size(vdfL      ),size(    vdfR  )) error(['ExpLMStrackingExpOnly: WARNING! vdfL     and vdfR     must be the same size. Check inputs!']); end;
if ~isequal(size(mchL      ),size(    mchR  )) error(['ExpLMStrackingExpOnly: WARNING! mchL     and mchR     must be the same size. Check inputs!']); end;
...
elseif strcmp(D.cmpInfo.localHostName,'jburge')
    ...
    disp(['ExpLMStrackingExpOnly: WARNING! running in stereoMode b/c D.cmpInfo.localHostName~=jburge-wheatstone. Rather D.cmpInfo.localHostName' D.cmpInfo.localHostName ]);
    D.stereoMode = 0;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%
disp(['ExpLMStrackingExpOnly: Starting tracking experiment with Gabor ' D.cmpInfo.localHostName]);
...
for t = 1:S.trlPerRun
    ...
    elseif strcmp(S.mtnType(1,1),'B') || strcmp(S.mtnType(1,1),'O') % BROWNIAN MOTION
        ...
        elseif  strcmp(mtnType,'BRX') || strcmp(mtnType,'ORX') % BROWNIAN X MOTION IN RE ONLY
            ...
            error(['ExpLMStrackingExpOnly: WARNING! unhandled mtnType=' mtnType]);
        end
        ...
        disp(['ExpLMStrackingExpOnly: WARNING! unhandled stmType= ' S.stmType(1,:)]);
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for t = 1:S.trlPerRun
    ...
    elseif bLostTrack == 1 % LOST TRACK (FROM MATLEAP)
        ...
        if S.lostTrackCnt(t,1) > 3
            ...
            Screen('CloseAll');
            sca
            disp(['ExpLMStrackingExpOnly: WARNING! Experiment quit b/c of MULTIPLE lost tracks']);
            return;
        end
    end
    ...
    if bEscKeyPressed == 1
        ...
        Screen('CloseAll');
        sca
        disp(['ExpLMStrackingExpOnly: WARNING! Experiment quit b/c of ESCAPE key press']);
        return;
    end
...
% SAVE DATA %
%%%%%%%%%%%%%
disp(['ExpLMStrackingExpOnly: SAVING DATA...']);
...
% else
%     for i = 1:5,
%     disp(['ExpLMStrackingExpOnly: WARNING! make sure to decide carefully whether or not to save all S.stmLE and S.stmRE fields w. expType=' expType(1,:)]);
%     end
% end
%
% if ismac == 1
