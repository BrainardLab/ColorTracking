function [S D] = ExpTTdetection(S,subjName,IPDmm,stmType, mtnType, indRnd, bUseFeedback, bSKIPSYNCTEST, bDEBUG)
...
%% Calculate the background setting 
...
for i = 1:size(indRnd,2)
    for t = 1:S.trlPerRun
        ...
        if strcmp(S.mtnType(1,1),'B') || strcmp(S.mtnType(1,1),'O') % BROWNIAN MOTION
            ...
            if      strcmp(mtnType,'BNM') || strcmp(mtnType,'ONM') % BROWNIAN MOTION IN XZ
