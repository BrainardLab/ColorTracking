function [S,bTrkBad,bEsc] = psyPresentTrialTrackingBinoLMS(D,S,t,msk1oF)
...
%       if size(msk1oF,2) == D.scrnXYpix(1) && size(msk1oF,1) == D.scrnXYpix(2)
    bUseMsk = 1;
%       else
%       error(['psyPresentTrialTrackingBinoLMS: WARNING! texture mask ''msk1oF'' not sized properly... Double check inputs']);
%       end
end
...
if S.bStatic == 0
    ...
    for f = 1:numFrm
        ...
        elseif strcmp(S.mtnType(1,:),'B0X') || strcmp(S.mtnType(1,:),'BLX') || strcmp(S.mtnType(1,:),'BRX') || ...  % BROWNIAN MOTION IN XY     
            ...
            mssYmmL = mssYmm; mssYmmR = mssYmm;
        else error(['psyPresentTrialTrackingBinoLMS: WARNING! unhandled mtnType=' num2str(S.mtnType(1,:)) '. WRITE CODE?!?']);
        end
